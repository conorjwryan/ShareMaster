...
`UploadStatusBar` (same file) is a floating bottom bar: progress while uploading → green completion message, with clipboard wording only when the destination's copy-on-upload setting is enabled (auto-clears after 4 s) → failures persist with an ✕. Uploads started from inside a bucket browser go straight to that destination, into the folder currently open (see the bucket browser section); uploads from the root list first show a `UploadDestinationPicker` sheet and use the destination's configured prefix.

**Status bars live in an overlay window** (`StatusOverlay.swift`): both `UploadStatusBar` and `DownloadStatusBar` are hosted in their own pass-through `UIWindow` one level below `.alert`, attached once from `DestinationListView.onAppear`. This keeps them visible above **sheets** — e.g. a download started from the file detail sheet — where a `safeAreaInset` bar on the NavigationStack (the previous design) gets covered. The window's `hitTest` passes touches through everywhere except the bars themselves (with an iOS 18+ subview-scan variant, since the hosting view otherwise swallows empty space).

**Presentation rule (bug happened twice):** every upload/download alert must hang off the **NavigationStack itself**, not the root list view — presentations from a covered root don't reliably appear while a bucket view is pushed.
...
- **`DownloadManager`** mirrors `UploadManager`: queued jobs run one at a time via `S3Service.download` (ranged/rate-limited), a `DownloadStatusBar` stacks with the upload bar in the `StatusOverlay` window (floating above sheets), a UIKit background task covers the post-backgrounding grace period, and the same mobile-data gate applies (see below). The detail sheet also shows an inline per-file progress bar via `progress(for:)` (`currentKey` distinguishes the transferring job from queued ones).
