...
- `listDirectory(config:prefix:continuationToken:pageSize:)` — one "directory" level at a time for the iOS bucket browser. Sends `delimiter=/` so S3 groups deeper keys into `CommonPrefixes` (returned as `S3Folder`s) server-side, and pages `pageSize` entries per request via `max-keys` + continuation token (the iOS browser passes 200; both providers cap `max-keys` at 1,000). **Each page costs exactly one LIST request regardless of how many keys it returns** — both AWS S3 and Cloudflare R2 bill LIST per request, not per key, which is why pages are large: small pages just multiply requests (verified for both; providers implement different subsets of the S3 spec and charge for different things — see [Provider policies](provider_policies.md)). Results keep S3's lexicographic order; don't re-sort in the service — continuation pagination depends on the order staying stable across pages. (The iOS browser's non-ascending sort orders fetch *all* pages first and only sort the completed set client-side — see [iOS](ios.md#bucket-browser-folders-sorting-paging-permissions).) Both listing parsers also read each object's `ETag` into `S3Object.etag` (XML-unescaped, quotes stripped, `nil` when absent) — the iOS download store compares it against the value recorded at download time to detect remotely changed files ([iOS downloads](ios.md#downloads--offline-files)).
